...
캐릭터 관리 상세의 숙제 체크는 약 800ms 디바운스 뒤 서버 세션 경로로 자동 저장된다. `app/character/page.tsx`의 저장 상태 알림은 홈 체크보드의 상태 표시와 별개다. v1.99부터 `ContentChecklist` 숙제 목록 안의 상시 상태 안내와 `document.body` 포털의 화면 아래 알림을 함께 사용한다. 완료 표시 뒤 새로고침해야 한다.
