...
캐릭터 관리 상세의 숙제 체크는 약 800ms 디바운스 뒤 서버 세션 경로로 자동 저장된다. `app/character/page.tsx`의 저장 상태 알림은 홈 체크보드의 상태 표시와 별개이며, v1.98부터 화면 아래 중앙에 표시해 상단 메뉴에 가리지 않도록 한다. 완료 표시 뒤 새로고침해야 한다.
