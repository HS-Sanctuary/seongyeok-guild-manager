...
export default function AdminPage() {
  ...
  const handleBroadcastBanner = async () => { 
    if (!bannerInput.trim()) return alert("입력해주세요."); 
    setIsSubmitting(true); 
    try { 
      await supabase.from('nexus_banners').update({ is_active: false }).eq('is_active', true); 
      await supabase.from('nexus_banners').insert([{ message: bannerInput, is_active: true }]); 
      alert("송출 성공!"); 
      setBannerInput(""); 
      fetchActiveBanner(); 
  ...
  const handleAddContent = async (type: string, name: string) => { 
    ...
    await supabase.from('nexus_contents').insert([{ type, name: finalName, short_name: name.substring(0,2), is_weekend: false, is_active: true }]); 
  ...
  const deleteTrade = async (id: number) => { 
    if (!confirm("이 교환 항목을 삭제하시겠습니까?")) return; 
